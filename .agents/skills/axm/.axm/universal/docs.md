...
`.axm/` 下有三类文档，对应三套 Frontmatter 骨架：
...
7. **注释**：YAML 中避免 `#` 注释，说明信息写在正文 §一节

## 四、命名规则

- **扩展名**：统一 `.md`（Markdown + axm-meta 契约，区别于根入口 `AGENTS.md` 和业务/代码事实中的 `.md` 文件）
...
- 未来如需在 CI 中启用，直接挂 `node .claude/skills/axm/scripts/validate.mjs` 即可
