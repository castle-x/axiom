...
详细写作规范见 axm-progress `references/bug-doc-guide.md`。

## 十二、工具链约束

- Markdown 渲染器隐藏 `axm-meta` HTML 注释块，Biome/Prettier 不处理其内容 → 零工具链成本
- 本规范的契约由 axm-maintain 的 `scripts/validate.mjs` 机械校验，不依赖 git hook
- 未来如需在 CI 中启用，直接挂 `node <axm-maintain-skill-path>/scripts/validate.mjs` 即可
