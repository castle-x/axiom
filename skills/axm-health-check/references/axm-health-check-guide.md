...
2. 跑 axm-maintain：
   - `node <axm-maintain-skill-path>/scripts/validate.mjs --target=<项目根>`
   - `node <axm-maintain-skill-path>/scripts/reindex.mjs --target=<项目根> --dry-run`
...
4. 不修改目标项目 `.axm/universal/`，除非用户明确要求变更通用契约；通用契约变更应回到 axm skill 仓库的 `skills/axm-init/templates/`。
5. 收尾运行 axm-maintain `reindex.mjs --dry-run`、必要时 `reindex.mjs --target=<项目根>`，再跑 `validate.mjs --target=<项目根>`。
