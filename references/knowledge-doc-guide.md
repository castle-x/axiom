...
`code-refs` 是知识文档的**事实锚点**，validate.mjs 会校验每条路径真实存在；若路径存在但指向目录，会给 WARN，建议尽量收敛到具体文件。
...
**坏的 code-refs**：
- 只指向宽泛目录：`src/agent/` ⚠️（能通过存在性检查，但 validate 会 WARN；优先写文件路径）
- 指向已删除文件 ❌（validate 会抓）
...
- [ ] `code-refs` 每条都是真实存在的路径；尽量指向文件，目录只作为宽泛锚点并会触发 WARN
