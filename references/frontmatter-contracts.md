...
> 本文是 `.axm/universal/docs.mdc` §二的**精简速查版**，供 AI 在 Phase 3 写新文档时快速对齐字段。遇到边界情况回到 docs.mdc 查全文。

## 三套骨架一览

| 骨架 | 用在哪 | 必填字段 | 识别标志 |
|---|---|---|---|
| **A 规范** | `universal/*.mdc` · `project/*.mdc`（不含 index） | `status` · `last-reviewed` · `owner` · `applies-to` | 有 `applies-to` |
| **B 知识** | `knowledge/**/*.mdc`（不含 index） | `status` · `last-reviewed` · `owner` · `depth` · `code-refs` | 有 `depth` + `code-refs` |
| **C 索引** | 所有 `index.mdc` | `status` · `last-reviewed` · `owner` · `entries` | 文件名为 `index.mdc`，有 `entries` |
...
related:                          # 可选
  - ../knowledge/frontend/overview.mdc
---
```
...
owner: your-team
entries:
  - path: overview.mdc           # 文件：*.mdc；子目录：<name>/
    title: 子系统速查
...
| `entries[].path` | string | 文件 `*.mdc` 或子目录 `<name>/` |
...
- 文件名 **kebab-case**（`editor-layout.mdc`）
- **禁止** 日期/版本号前缀（`2026-04-22-plan.mdc` ✗，`v2-design.mdc` ✗）
- 日期严格 `YYYY-MM-DD`
- 路径统一 `/`
...
2. index.mdc 的 `entries` 忘了加新文件（孤儿警告）
3. `last-reviewed` 随手写成 `2026/05/07`（应 `-`）
4. `applies-to` 漏写（规范文档）
5. 把 index.mdc 写成骨架 A（`applies-to` 代替了 `entries`）
