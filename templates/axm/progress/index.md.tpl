...
- **bug 文档（`<initiative>/bugs/bug-YYYY-MM-DD-<slug>.md`）** 记录单条 BUG，必须是 `bugs/` 下的直接子文件，必须含优先级、复现步骤、修复验收标准与生命周期状态；BUG 必须挂在某个 initiative 下，禁止在 `progress/` 顶层建 `bugs/`，禁止在 `bugs/` 下再建嵌套目录；若无归属主题应先新建 initiative
